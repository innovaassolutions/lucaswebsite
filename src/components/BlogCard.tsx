...
export default function BlogCard({ post, featured = false }: BlogCardProps) {
  ...
  if (featured) {
    return (
      <Link href={`/blog/${post.slug}`} className="block group">
        <article className={`card-glow rounded-xl overflow-hidden ${rarityClasses[post.rarity]}`}>
  ...
  return (
    <Link href={`/blog/${post.slug}`} className="block group">
      <article className={`card-glow rounded-xl overflow-hidden h-full ${rarityClasses[post.rarity]}`}>
        {/* Top gradient accent */}
